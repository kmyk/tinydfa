 ...
 *         some abbreviations . + ?
 */
vector<vector<pair<int, int> > > construct_epsilon_nfa(const string & re, const string & alphabet) {
    // make the inverse table of alphabet
    array<int, 256> lookup;
    fill(ALL(lookup), -1);
    REP (i, alphabet.size()) {
        char c = alphabet[i];
        assert (string("()|*.+?").find(c) == string::npos);
        assert (lookup[c] == -1);
        lookup[c] = i;
    }
 ...
 */
vector<vector<vector<int> > > remove_epsilon_moves(const vector<vector<pair<int, int> > > & enfa, int alphabet_size) {
    constexpr int EPSILON = -1;
 ...
 */
pair<vector<vector<int> >, vector<bool> > apply_subset_construction(const vector<vector<vector<int> > > & nfa, int alphabet_size) {
...
pair<vector<vector<int> >, vector<bool> > construct_dfa_from_regex(const string & re, const string & alphabet) {
